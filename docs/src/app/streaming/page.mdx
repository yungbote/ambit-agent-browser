...
The existing stream carries full-window JPEG frames with a `surface` object: `kind: "browser-window"`, `coordinateSpace: "display-pixels"`, a UUID `generation`, physical `width` and `height`, `originX: 0`, `originY: 0`, `deviceScaleFactor: 2`, and `cursorIncluded` indicating whether the native cursor is present. These frames omit CDP page-frame metadata. Native UI scale is fixed at 2; Chromium page zoom and page screenshots retain their independently measured CSS geometry.
...
CLI and MCP automation continue to use their existing page tools. After a handoff, commands that name their target run; input addressed to the focused element or the pointer waits for a fresh page observation. A layout change keeps element refs and needs no new observation. The native driver observes actual window/page focus rather than assuming its last selected tab is still visible. Ambiguous focus and pinned-tab mismatches require explicit selection. Their refusal, `browser_active_page_ambiguous`, lists the open tabs in `data.tabs` (at most 20, each with its `tabId`, any `label`, its `title` cut to 100 characters, its URL `origin` and whether it is `active`) and counts them in `data.tabCount`; it selects none of them. A native window close keeps the daemon available for an explicit `open`; that creates a new browser, without claiming to restore closed tabs. The CLI `close` command ends the session and its display.
...
### Binary frames and presentation updates

An upgrade-bound window presenter can send `{"type":"presentation","width":733,"height":896}` on the same connection. The viewer identity stays fixed, and existing presentation custody determines whether that size applies. This does not grant input. Controlled resizing still uses the host input sequence.

Negotiate `patches=1` for JPEG damage rectangles and `frames=binary` to avoid base64. The binary message is a four-byte big-endian JSON-header length, the UTF-8 header, then JPEG bytes in declaration order. The header is bounded at 64 KiB and the message at 12 MiB. Whole frames carry `byteLength`; patches carry their own `byteLength` plus `x`, `y`, `width`, `height`, `sourceX` and `sourceY`. Byte lengths must exactly consume the payload. Draw the source crop at its destination; padding supplies the JPEG decoder's neighboring pixels.

A delta's `baseSeq` must equal the last painted frame with matching surface generation and dimensions. Fresh viewers and skipped dependencies receive a whole frame. With `pacing=ack`, acknowledge only after the complete frame is painted. Other records remain text JSON; existing text viewers are unchanged.
