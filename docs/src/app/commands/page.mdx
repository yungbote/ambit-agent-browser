...
`run-playwright` executes an async JavaScript body using the real `page`, `context`, and `browser` objects from the already-open Chromium instance. Its default page is the native driver's current target. The MCP equivalent is `agent_browser_run_playwright` with `code`, optional `targetId`, and optional `timeoutMs`.
