...
agent-browser run-playwright <code>    # Async Playwright body in the current browser
...
## Playwright in the current browser

`window new` opens a window in its own cookie context; closing that window's last tab discards the context, as closing a private window does. `window new --shared`, or MCP `agent_browser_window_new` with `shared: true`, opens one in the persistent profile instead. A Playwright client with context adoption represents each tab's real context; the stock 1.62.1 client cannot, so it refuses to attach while an isolated window is open. Native tools work in every window; closing an isolated window admits the stock client again. Existing windows and credentials are never moved or copied.

On Linux, private window streaming defaults to ANGLE software GLES for WebGL with software compositing, so the stream still sends only damaged regions. Explicit browser arguments override this preset, and `--webgpu` keeps its existing backend. Fresh driver-owned profiles open `about:blank`; retained profiles and caller-selected startup behavior are preserved.

`run-playwright` executes an async JavaScript body using the real `page`, `context`, and `browser` objects from the already-open Chromium instance. The body runs in Node, so `document` and `window` exist only inside `page.evaluate()`. Its default page is the native driver's current target. The MCP equivalent is `agent_browser_run_playwright` with `code`, optional `targetId`, and optional `timeoutMs`.
...
Use `--target <id>` to select another existing CDP target and `--timeout-ms <ms>` for a deadline from 1 through 120000 milliseconds (default 30000). Return a JSON value of at most 2 MiB. Console output is returned separately as diagnostics, capped at 64 KiB. The body is limited to 1 MiB.

The host must provide Node and a qualified `playwright-core` installation. Version 1.62.1 supports the required `noDefaults` CDP attachment, which retains native focus, media and download settings. Set `AGENT_BROWSER_PLAYWRIGHT_MODULE` to its absolute `index.mjs` path; `AGENT_BROWSER_NODE_PATH` and `AGENT_BROWSER_PLAYWRIGHT_RUNNER` optionally select the installed executable and runner. Execution never downloads a browser or installs dependencies.

The operation shares the profile, tabs and native window. Real mouse events use the native display owner; DOM calls such as `element.click()` do not fabricate pointer movement. Native typing is used for ordinary keys and inserted text; browser editing/composition and touch keep their CDP semantics. Human takeover, timeout and caller disconnect settle the program's process group and temporary attachment before handing back custody. Chrome remains owned by the native daemon. A program that fails before it issues any Playwright call did nothing in the browser: it fails with `browser_program_error`, the thrown error and its line in `data.error`, and `data.pageCallsIssued` of 0, so fix it and run it again. Any other failed or interrupted program may already have performed effects and is never replayed automatically; one that failed after its calls reports `data.pageCallsIssued` and `data.lastPageCall`. This is ordinary authorized workspace code, not a new hostile-code sandbox.
...
`download` saves the first newly observed download from the clicked page or its frames, selected by start order. `wait --download` handles the oldest retained unreported download from the current page or its frames, including downloads completed before the wait. An optional path saves the actual file there; otherwise the result contains its existing absolute path. Canceled downloads return an error and are not returned again by later waits. Recent terminal history is limited to 256 records.

Agent-browser-managed Chromium observes downloads throughout its lifetime. Use `--download-path <dir>` (or `AGENT_BROWSER_DOWNLOAD_PATH` env) to set its download directory. Without it, downloads go to a temporary directory that is deleted when the browser closes. An attached browser's active context is configured only by an explicit `download`, using its destination's parent directory as retained storage; `wait --download` alone does not change download settings.
...
`tab list --json` also reports each tab's CDP `targetId`, and target ids are accepted anywhere a tab ref is accepted (`tab <targetId>`, `tab close <targetId>`). Unlike `t<N>` ids, which are per-daemon counters, target ids stay stable across daemon restarts. Each session is bound to its active tab by target id and returns to it after a daemon restart; with `--pin-tab` the binding is strict and a closed bound tab produces a `tab_gone` error, before the command acts, instead of a silent fallback. JSON errors include <code>code: "tab_gone"</code>, <code>data.targetId</code>, optional sanitized <code>data.lastUrl</code>, and the open tabs in <code>data.tabs</code> and <code>data.tabCount</code>. A command whose bound tab closes while it runs fails with <code>code: "tab_closed_during_command"</code> and the same data, since it may already have acted. A tab ref that names no open tab fails with <code>code: "tab_not_found"</code> and the open tabs. Batch output carries the recovery object under <code>result</code>. See [CDP Mode](/cdp-mode) for the multi-session workflow.
...
The `frame` command accepts element refs (`@e3`), CSS selectors (`"#my-iframe"`), or frame name/URL. A `frame` selection lasts as long as the page it was made on. After that page navigates or reloads (or while another tab is shown), a command that would act inside the frame is refused with `browser_observation_stale` and nothing is done; select the main frame or a frame of the current page, then send it again.
...
--theme <dark|light>     # Browser theme at launch: window UI and pages
...
--require-sandbox        # Require sandboxed local Chrome without unsandboxed fallback
--require-daemon         # Require an existing compatible daemon without spawning or restarting
...
`agent-browser [options] daemon` runs in the invoking process for a host supervisor and retains stderr. Termination cancels active commands and pending Chrome startup without retrying, and allows one second for state saving before browser cleanup; a timeout is reported on stderr. Clients use `--require-daemon` (or `AGENT_BROWSER_REQUIRE_DAEMON=1`, config `requireDaemon`) with matching session and daemon settings. See [Supervised daemon](/sessions#supervised-daemon) for lifecycle and configuration behavior.
...
Each tool has typed fields such as `url`, `selector`, `text`, `key`, `session`, `allowedDomains`, and `idleTimeout`, so MCP clients show meaningful approval prompts instead of raw command arrays. The common `allowedDomains` array maps to `--allowed-domains` and activates the same WebRTC containment and launch-mode restrictions. The common `idleTimeout` string maps to `--idle-timeout`, accepts values such as `30s`, `5m`, `1h`, or raw milliseconds, and accepts `0` to disable idle shutdown. The common `requireDaemon` boolean maps to `--require-daemon`, preventing client startup or restart of supervised daemons. The common `requireSandbox` boolean maps to `--require-sandbox` and enforces the same local Chrome launch policy. The foreground `daemon` command is intentionally a host lifecycle command without an MCP tool. Each tool also accepts `extraArgs` for advanced CLI flags and exact CLI parity. Tool discovery is paginated and includes read-only/open-world annotations so modern MCP clients can load the large typed surface incrementally.
