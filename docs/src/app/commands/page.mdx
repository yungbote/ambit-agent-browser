...
Ordinary new windows share the persistent profile. `window new --isolated`, or MCP `agent_browser_window_new` with `isolated: true`, retains separate native cookies. Playwright cannot represent pre-existing isolated contexts correctly and refuses attachment while their windows remain open. Native tools still work in those windows; closing them permits Playwright attachment again. Existing windows and credentials are never moved or copied.
