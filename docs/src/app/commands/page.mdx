...
Use `--target <id>` to select another existing CDP target and `--timeout-ms <ms>` for a deadline from 1 through 120000 milliseconds (default 30000). Return a JSON value of at most 2 MiB. Console output is returned separately as diagnostics, capped at 64 KiB. The body is limited to 1 MiB. `process.stdout` is reserved for the result protocol.
