...
On Linux, private window streaming defaults to ANGLE software GLES for WebGL. Explicit browser arguments override this preset, and `--webgpu` keeps its existing backend. Fresh driver-owned profiles open `about:blank`; retained profiles and caller-selected startup behavior are preserved.
