...
The browser stays running across commands so these feel like a single session. By default, an inactive daemon saves configured restore state, closes its headless browser, and exits after one hour; the next command starts it again. Without `--restore` or another restore key, shutdown discards transient browser state and open tabs. Dashboard mouse, keyboard, and touch input count as activity. Headed browsers, Safari and iOS WebDriver sessions, and user-attached browsers are exempt from the default; provider-owned cloud browsers are not. Use `--idle-timeout <time>` or `AGENT_BROWSER_IDLE_TIMEOUT_MS` to tune the timeout, and use `0` to disable it. Still run `agent-browser close` (or `close --all`) when you're done. If a command reports `Command outcome unknown`, it may already have been applied. Inspect current browser or external state before repeating it; transport failures after sending are never replayed automatically. When a host supervisor owns the session, it starts `agent-browser daemon`; use `--require-daemon` on clients with the same daemon settings so a lost or incompatible daemon fails without spawning or restarting one. Use `--require-sandbox` when the host supports sandboxed Chrome; it disables automatic unsandboxed fallback and rejects disabling arguments and externally owned browsers. Termination cancels active work and pending Chrome startup without retrying, and gives state saving one second; a timeout is reported on stderr. See `references/session-management.md` for the foreground lifecycle and sandbox contracts.

If a command returns `browser_controlled_by_user`, wait for the user to release browser control before continuing. If it returns `browser_control_outcome_unknown`, do not replay the input; inspect the current page after control is released. The existing workflow remains paused until the host resumes it. After human control, commands that name their target (URL, tab, selector, ref, script) run as usual. Key and text input without a selector, mouse input at the current pointer, a dialog accept or dismiss, and a Playwright program are refused once with `browser_observation_required` and nothing is sent; host-bound MCP attaches fresh feedback to the refusal. Inspect it, then send the input again or address the element by selector or ref.
...
`window new` opens a window in its own cookie context; closing its last tab discards that context. Use `window new --shared` or MCP `agent_browser_window_new` with `shared: true` when the window should carry the signed-in profile. Isolated windows are always available to native tools; a Playwright client without context adoption refuses to attach while one of them is open, because reporting its pages as the profile would misrepresent authentication state. No existing profile or cookie state is migrated.

Use `agent-browser run-playwright --stdin` or MCP `agent_browser_run_playwright` for ordinary async JavaScript bodies using the existing `page`, `context`, and `browser`. Open the native browser first. Use `return` for a JSON result; the default page is the actual current native tab, and `targetId` selects an exact existing tab. The body runs in Node: `document` and `window` exist only inside `page.evaluate()`. Do not launch another browser or copy its profile. Locators, frames, popups, network listeners and async composition retain Playwright semantics. Real mouse input follows the same native window owner; `page.evaluate(() => element.click())` does not invent mouse movement. Read [references/playwright.md](references/playwright.md) for runtime requirements, outputs and cancellation. `browser_program_error` means the program failed before it issued any Playwright call and did nothing in the browser: fix it from `data.error` and run it again. Any other failed or interrupted program may already have changed external state and must never be replayed automatically.
...
When several sessions share one Chrome over `--cdp <port>`, add `--pin-tab` so each session sticks to its own tab. Every session remembers its bound tab across daemon restarts; with `--pin-tab` a command addressed to a bound tab that was closed is refused with a `tab_gone` error before it acts, instead of acting on another session's tab. JSON output includes `"code": "tab_gone"`, `data.targetId`, an optional sanitized `data.lastUrl`, and the open tabs in `data.tabs` for recovery. A command whose bound tab closes while it runs fails with `tab_closed_during_command` and the same data: it may already have acted, so inspect before retrying. Recover with `tab new <url>` or pick a tab from `data.tabs` or `tab list`. The flag is sticky per session, so pass it once (`--no-pin-tab` turns it off again). See `references/session-management.md` for details.
...
Authorized Product hosts can mediate laptop files through the internal human-control file protocol: pending chooser or exact drop destination, canonical staged-file delivery, and observed completed download metadata. Agents continue using the existing `upload`, `download`, and `waitfordownload` tools. Human file operations require the active controller and are intentionally absent from the CLI and MCP catalog. See [references/streaming.md](references/streaming.md) for the Product contract.

A Linux host may enable `AGENT_BROWSER_WINDOW_STREAM=1` to show the actual Chromium window while the normal CLI and MCP tools automate that same browser. Human window input and agent page coordinates use different spaces: stream surfaces are physical display pixels at native UI scale 2; screenshots from page tools retain their measured viewport CSS mapping and page zoom. After a user handoff, input addressed to the focused element or the pointer waits for a fresh snapshot or screenshot (host-bound MCP supplies it with `browser_observation_required`); commands that name their target run. A dock layout change keeps refs and needs no new observation. Native tab focus is re-observed before page commands, and ambiguous or pinned-tab mismatches require explicit tab selection: the `browser_active_page_ambiguous` refusal lists the open tabs in `data.tabs` (`tabId`, `title`, `origin`, `active`), so select one with `tab <tabId>` instead of listing tabs first. See [references/streaming.md](references/streaming.md) for the window protocol, helper requirements, clipboard limits and lifecycle. Viewers may negotiate binary damage frames and update presentation dimensions on the same connection; CLI/MCP page tools keep their existing commands.
