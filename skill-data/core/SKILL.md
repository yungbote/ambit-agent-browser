...
`window new` shares the persistent browser profile. Use `window new --isolated` or MCP `agent_browser_window_new` with `isolated: true` only when separate cookies are intended. Existing isolated windows remain available to native tools; Playwright attachment is refused while those unsupported contexts are open, because reporting their pages as the default context would misrepresent authentication state. No existing profile or cookie state is migrated.
