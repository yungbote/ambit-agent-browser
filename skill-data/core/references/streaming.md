...
Before ordinary page actions, the driver observes native window focus and actual page visibility. Ambiguous focus and pinned-tab mismatches require explicit tab selection. Their refusal, `browser_active_page_ambiguous`, lists the open tabs in `data.tabs`: at most 20, in tab order (an active tab beyond them takes the last place), each with its `tabId`, any `label`, its `title` (whitespace collapsed, cut to 100 characters and ending in `…` when cut), its URL reduced to the `origin` (only the scheme for URLs without one, such as `about:` or `data:`) and whether it is `active`. `data.tabCount` counts every open tab. The refusal selects no tab; titles are read from each page while its visibility is observed. After a human handback, commands that name their target run; input addressed to the focused element or the pointer waits for a new snapshot or screenshot, and host-bound MCP supplies that feedback with `browser_observation_required`. A layout change keeps element refs. Pointer input and unfenced points admitted before a layout change cannot use a later observation to replay stale coordinates; they are refused as `browser_observation_stale`. Native window closure leaves the existing daemon available for an explicit `open`, which creates a new browser with a fresh target. `close` ends that session and reaps its display. Unlabelled stream loss still does not prove permanent closure.
