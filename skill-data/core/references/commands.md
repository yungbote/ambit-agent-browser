...
`set theme` changes the theme of a running session. Every page of its automated browser switches now, including background tabs, tabs a person opened and popups, and tabs opened later get it too. In a qualified managed Linux image, the private display’s standard system settings switch the window UI live without opening a settings tab or relaunching Chrome. Older images take the theme at the next launch; explicitly pinned custom UI settings remain authoritative; every launch the daemon performs, including sign-in and hand-back, uses the session theme unless that launch names its own. The result data carries `theme`, `pages` (`live`, or `next_launch` when no automated browser runs, as during sign-in) and `ui` (`live` after the private settings acknowledge the update, `next_launch` for the fallback, `pinned` for explicit custom UI settings (including a set `GTK_THEME`), or `none` for a headless or attached browser). It never starts a daemon or launches a browser; with no running session it fails with `browser_runtime_unavailable` and changes nothing. It is not an agent action, so it is accepted while a person controls the browser or signs in, with a JavaScript dialog open or an ambiguous active tab, and it never counts as the fresh observation a handoff requires. The MCP tool is `agent_browser_set_theme` in the `mobile` profile, and `agent_browser_open` accepts an optional `theme`.
...
AGENT_BROWSER_WINDOW_STREAM="1"              # Actual Chromium window on an owned Linux X11 display
AGENT_BROWSER_DISPLAY_HELPER="/path/browser-display" # Default: beside the native executable
...
For an owned full-window stream, the ordinary page tools still use viewport CSS coordinates and current page zoom. Human input uses the surface's physical display pixels. After native window control, CLI input addressed to the focused element or the pointer requires a fresh `snapshot` or `screenshot`; host-bound MCP supplies a fresh observation in its typed refusal. Commands that name their target, and every command after a layout change, run without one. The host's fixed launch configuration is applied only after custody and observation checks, in the same command boundary. See [streaming.md](streaming.md) for the full-window host protocol. Window control intentionally remains an internal host operation rather than an agent-facing MCP tool.
