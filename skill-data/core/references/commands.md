...
For an owned full-window stream, the ordinary page tools still use viewport CSS coordinates and current page zoom. Human input uses the surface's physical display pixels. After native window control or layout changes, a legacy CLI page action requires a fresh `snapshot` or `screenshot`; host-bound MCP supplies a fresh observation in its typed refusal. The host's fixed launch configuration is applied only after custody and observation checks, in the same command boundary. See [streaming.md](streaming.md) for the full-window host protocol. Window control intentionally remains an internal host operation rather than an agent-facing MCP tool.
