...
The temporary transport routes actual mouse input through the native display owner in window mode. It does not interpolate movement, delay actions for animation or manufacture pointer activity for DOM evaluation. Keyboard text and ordinary key events use the native owner; browser editing commands and composition retain CDP semantics. Touch remains CDP input. No new viewer protocol or public debugging URL is created.

Return a JSON-serializable value (maximum 2 MiB). `undefined` becomes `null`. Console output is returned as diagnostic text, capped at 64 KiB with an explicit truncation flag. Do not write directly to `process.stdout`, which carries the runner result. Code is limited to 1 MiB and the operation deadline is 1 through 120000 milliseconds, defaulting to 30000. The native owner settles processes and input after the deadline before releasing custody.

Human takeover, caller disconnect, timeout and native shutdown stop the runner's process group and detach its temporary connection while retaining Chrome. The current action is not rolled back. A rejected program is marked `browser_operation_rejected`; after program execution starts, an exception, serialization failure, cancellation or lost result is `browser_operation_outcome_unknown`. Inspect current browser and external state before choosing another action; never automatically replay the body. Deliberately issued browser/page commands retain their real effects.
